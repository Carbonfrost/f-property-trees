...
namespace Carbonfrost.Commons.PropertyTrees.ValueSerializers {
    ...
    sealed class NullableValueSerializer<T> : ValueSerializer {

        public override object ConvertFromString(string text, Type destinationType, IValueSerializerContext context) {
            if (text != null && typeof(T) == Nullable.GetUnderlyingType(destinationType)) {
                text = text.Trim();
                if (text.Length == 0) {
                    return null;
                }

                return ValueSerializer.GetValueSerializer(typeof(T)).ConvertFromString(text, destinationType, context);
            }

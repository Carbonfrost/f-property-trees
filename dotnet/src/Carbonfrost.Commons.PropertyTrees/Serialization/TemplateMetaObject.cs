    ...
    partial class TemplateMetaObject : PropertyTreeMetaObject {
        ...
        public override PropertyTreeMetaObject BindEndObject(IServiceProvider serviceProvider) {
            if (this.Parent == null) {
                ...
                var templ = new PropertyTreeTemplate(_items.ToArray());
                var templateType = typeof(Template<>).MakeGenericType(instType);

                this.component = Activator.CreateInstance(templateType, new object[] { templ });
            } else {
